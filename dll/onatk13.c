...
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk13.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk13_cmd.txt"
...
static void diag(void){uint32_t pl=RU(PLAYER);if(!pl){L("[診斷]未進場");return;}if(!farr(pl)){L("[診斷]無怪陣列");return;}
 ...
 for(int c=0;c<8;c++){uint32_t best=0;long bd=1<<30;
  ...
  L("[診斷]怪%d objid=0x%X @(%d,%d) 曼距=%ld 格距=%d 牆=%d 視線=%d 狀態=%d 活=%d",
    c,RU(best+OFF_ID),mx,my,bd,cheb(px,py,mx,my),wall(mx,my),los_clear(px,py,mx,my),RB(best+OFF_STATE),alive(best));}
}
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk13 啟動 pid=%lu ====",GetCurrentProcessId());
 ...
 for(;;){Sleep(150);cmd();
  ...
  if(hunt&&g_hw){htick++;if(htick>=3){htick=0;   /* 每~450ms 檢查 */
   ...
   if(e){int mx=RI(e+OFF_X),my=RI(e+OFF_Y);dd=labs(mx-px)+labs(my-py);dch=cheb(px,py,mx,my);
    if(dd<=MAXDIST&&(!use_los||los_clear(px,py,mx,my)))valid=1;}
