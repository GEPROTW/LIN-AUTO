...
#define OFF_ID 0x0C
#define OFF_AI 0x10
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk4.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk4_cmd.txt"
...
static int mode=0,hunt=0;static uint32_t lock_oid=0;
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk4 啟動 pid=%lu ====",GetCurrentProcessId());
 ...
 for(;;){Sleep(150);cmd();
  ...
  /* 自動打怪: 每~1.2秒, FUN_0040e790 打最近通暢怪(視線過濾避免追牆卡住) */
  if(hunt&&g_hw){tick++;if(tick>=8){tick=0;
   ...
   /* 鎖定的怪若視線被擋或消失, 換一隻 */
   if(e&&!los_clear(px,py,RI(e+OFF_X),RI(e+OFF_Y)))e=0;
   if(!e){e=nearest_clear(pl,px,py,1);if(e){lock_oid=RU(e+OFF_ID);L("[獵]新目標objid=0x%X",lock_oid);}}
   if(e){L("[獵]0x%08X objid=0x%X @(%d,%d) 距=%ld 視線OK→FUN_0040e790",e,RU(e+OFF_ID),RI(e+OFF_X),RI(e+OFF_Y),labs(RI(e+OFF_X)-px)+labs(RI(e+OFF_Y)-py));
