/* oatk2.c — 真‧攻擊觸發 (32-bit)
 * 配方(來自 FUN_005ABC90 反組譯):
 *   cmd = op_new(0x1C0)                      ; 0x0079A150 __cdecl(size)
 *   FUN_004C7D80(ecx=cmd, 0,0x4E0,0,0,x,y,0,0)  ; __thiscall, callee 清堆疊
 *   (*(*(void**)cmd)[1])(ecx=cmd)            ; vtable+4 = 送出攻擊封包
 * 指令: oatk2_cmd.txt 寫 "atk" → 對最近的妖魔送攻擊(維持數秒)
 * gcc -shared -O2 -s -o oatk2.dll oatk2.c
 */
#include <windows.h>
#include <stdio.h>
#include <stdint.h>
...
#define OFF_Y 0x38
#define OFF_AI 0x10
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\oatk2.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\oatk2_cmd.txt"
#define MODLO 0x00400000u
#define MODHI 0x00F0B000u
#define OP_NEW  0x0079A150u
#define FN_CTOR 0x004C7D80u
#define ATK_OPCODE 0x4E0
static FILE *g;
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== oatk2 啟動 pid=%lu ====",GetCurrentProcessId());
