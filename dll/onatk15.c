...
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk15.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk15_cmd.txt"
...
static void diag(void){uint32_t pl=RU(PLAYER);if(!pl){L("[診斷]未進場");return;}if(!farr(pl)){L("[診斷]無怪陣列");return;}
 ...
 for(int c=0;c<10;c++){uint32_t best=0;long bd=1<<30;
  ...
  L("[診斷]實體%d objid=0x%X 曼距=%ld 格距=%d 類別[+CD]=%d 名[%s] 主人欄位=%s 視線=%d 活=%d",
    c,RU(best+OFF_ID),bd,cheb(px,py,RI(best+OFF_X),RI(best+OFF_Y)),RB(best+0xCD),nm,
    own>=0?"有(是寵物?)":"無",los_clear(px,py,RI(best+OFF_X),RI(best+OFF_Y)),alive(best));
  if(own>=0)L("        ↑ 主人欄位在 +0x%X",own);}
}
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk15 啟動 pid=%lu ====",GetCurrentProcessId());
