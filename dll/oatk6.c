/* oatk6.c — 真攻擊 (opcode 0x3C33 + 方向 a3)
 * 配方(側錄自真實攻擊): FUN_004C7D80(ecx=cmd, 0,0x3C33,dir,0, mx,my, 0,0); 執行=vtable+4
 *   dir = 玩家→怪 的 8 方向(0-7)。 gcc -shared -O2 -s -o oatk6.dll oatk6.c
 */
#include <windows.h>
#include <stdio.h>
#include <stdint.h>
...
#define OFF_Y 0x38
#define OFF_AI 0x10
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\oatk6.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\oatk6_cmd.txt"
...
static void attack(uint32_t pl,int px,int py,uint32_t e){
 ...
 ctor(cmd, 0, ATK, dir, 0, px, py, 0, 0);      /* this=cmd, a1..a8 */
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== oatk6 啟動 pid=%lu ====",GetCurrentProcessId());
