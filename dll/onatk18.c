...
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk18.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk18_cmd.txt"
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk18 啟動 pid=%lu ====",GetCurrentProcessId());
 ...
 for(;;){Sleep(150);cmd();
  ...
  if(hunt&&g_hw){htick++;if(htick>=2){htick=0;   /* 每~300ms 掃描 */
   ...
   if(moving){ka=0;/* 移動中: 不送指令 */}
    ...
    if(cch<=BOWRANGE){ /* 正在射: 只有畫面內近很多的怪才改打, 否則專打不補點 */
