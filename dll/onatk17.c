...
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk17.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk17_cmd.txt"
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk17 啟動 pid=%lu ====",GetCurrentProcessId());
 ...
 for(;;){Sleep(150);cmd();
  ...
  if(hunt&&g_hw){htick++;if(htick>=3){htick=0;   /* 每~450ms 檢查 */
   ...
   if(e){int mx=RI(e+OFF_X),my=RI(e+OFF_Y);dd=labs(mx-px)+labs(my-py);dch=cheb(px,py,mx,my);
    if(dd<=MAXDIST&&dch>=MINRANGE){
     if(dch>BOWRANGE)valid=1;                              /* 遠: 走去接近(尋怪), 不要求視線 */
     else if(!use_los||los_clear(px,py,mx,my))valid=1;     /* 進射程: 視線通暢才能射 */
    }}
   ...
   if(moving){ka=0;/* 移動中: 維持現狀, 絕不送指令 */}
    ...
    if(lock_oid&&valid){
     ...
     /* 沒目標: 先挑最近視線通暢真怪; 沒有就走向最近真怪(尋怪roam) */
     long bd;uint32_t best=nearest_clear2(pl,px,py,use_los,&bd);
     int roam=0;
     if(!best&&use_los){best=nearest_clear2(pl,px,py,0,&bd);roam=1;} /* 無通暢的→走去找最近真怪 */
     if(best){lock_oid=RU(best+OFF_ID);ka=0;lostc=0;long bch=cheb(px,py,RI(best+OFF_X),RI(best+OFF_Y));
      L("[獵]%s0x%08X objid=0x%X 曼距=%ld 格距=%ld",roam?"尋怪走向":"新目標",best,lock_oid,bd,bch);PostMessageW(g_hw,WM_ATK,(WPARAM)lock_oid,0);}
    }
