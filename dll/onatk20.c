...
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk20.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk20_cmd.txt"
...
/* 畫面內(格距<=VIEWRANGE) 最近的視線通暢活真怪. *outch 帶回格子距離 */
static uint32_t nearest_view(uint32_t pl,int px,int py,int*outch){uint32_t best=0;int bch=9999;
 for(int i=0;i<gl;i++){uint32_t e=RU(gb+i*4);if(!heap(e)||e==pl)continue;if(RU(e+OFF_AI)==0)continue;
  ...
  if(ch<MINRANGE||ch>VIEWRANGE)continue;if(use_los&&!los_clear(px,py,mx,my))continue;
...
static int mode=0,hunt=0,sensing=0;static uint32_t lock_oid=0;static int htick=0,ka=0;static int lastpx=0,lastpy=0;static int lostc=0;
static void cmd(void){FILE*f=fopen(CMDPATH,"r");if(!f)return;char b[64]={0};fgets(b,64,f);fclose(f);remove(CMDPATH);
 ...
 else if(strncmp(b,"sense",5)==0){sensing=1;hunt=0;L("[CMD]sense 純感知模式(只列真怪, 不攻擊)");}
 else if(strncmp(b,"los0",4)==0){use_los=0;L("[CMD]視線過濾=關(打最近怪不管牆)");}
 else if(strncmp(b,"los1",4)==0){use_los=1;L("[CMD]視線過濾=開(只打視線通暢)");}
 else if(strncmp(b,"hunt",4)==0){hunt=1;sensing=0;lock_oid=0;L("[CMD]hunt 自動打怪(視線過濾=%d)",use_los);}
 else if(strncmp(b,"stop",4)==0){hunt=0;sensing=0;L("[CMD]stop");}
 else if(strncmp(b,"s22",3)==0){mode=2;L("[CMD]s22 單發直送");}
 else if(strncmp(b,"atk",3)==0){mode=1;L("[CMD]atk 單發原生");}}
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk20 啟動 pid=%lu ====",GetCurrentProcessId());
 ...
 for(;;){Sleep(150);cmd();
  ...
  if(hunt&&g_hw){htick++;if(htick>=2){htick=0;   /* 每~300ms 掃描 */
   ...
   /* 每偵: 畫面內(格距<=VIEWRANGE)最近的視線通暢活真怪 */
   ...
   int curValid=0;
   if(cur&&cch>=MINRANGE&&cman<=MAXDIST){
    if(cch>BOWRANGE)curValid=1;
   ...
   if(moving){ka=0;/* 移動中: 不送指令 */}
    ...
    if(best){lock_oid=RU(best+OFF_ID);ka=0;
     ...
     if(r&&cheb(px,py,RI(r+OFF_X),RI(r+OFF_Y))>BOWRANGE){lock_oid=RU(r+OFF_ID);ka=0;
