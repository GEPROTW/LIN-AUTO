...
#define OFF_ID 0x0C
#define OFF_AI 0x10
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk6.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk6_cmd.txt"
...
static void do_send22(uint32_t oid){uint32_t pl=RU(PLAYER);if(!pl)return;
 uint32_t e=find_by_oid(pl,oid);if(!e||!ent(e)){L("[主]目標已消失");return;}
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk6 啟動 pid=%lu ====",GetCurrentProcessId());
