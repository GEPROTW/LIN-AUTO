...
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk12.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk12_cmd.txt"
...
static int mode=0,hunt=0;static uint32_t lock_oid=0;static int htick=0,ka=0;static int lastpx=0,lastpy=0;static int lostc=0;
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk12 啟動 pid=%lu ====",GetCurrentProcessId());
 ...
 for(;;){Sleep(150);cmd();
  ...
  if(hunt&&g_hw){htick++;if(htick>=3){htick=0;   /* 每~450ms 檢查 */
   ...
   long dd=999;int valid=0;
   if(e){int mx=RI(e+OFF_X),my=RI(e+OFF_Y);dd=labs(mx-px)+labs(my-py);
    if(dd<=MAXDIST&&los_clear(px,py,mx,my))valid=1;}
   if(lock_oid&&!valid){lostc++;if(lostc>=3){lock_oid=0;lostc=0;}}else lostc=0; /* 寬限3次才放棄 */
   if(moving){ka=0;/* 移動中: 維持現狀, 絕不送指令 */}
   else{
    if(lock_oid&&valid){
     if(dd<=BOWRANGE){ka=0;/* 進射程正在射: 認定專打, 不切換不補點 */}
     else{
      /* 走去打途中: 只有近很多(>=8格)的怪才改道, 否則繼續接近(卡住才補點) */
      long bd;uint32_t best=nearest_clear2(pl,px,py,1,&bd);
      if(best&&bd+8<=dd&&RU(best+OFF_ID)!=lock_oid){lock_oid=RU(best+OFF_ID);ka=0;lostc=0;
       L("[獵]改打近很多的怪 objid=0x%X 距=%ld(原距%ld)",lock_oid,bd,dd);PostMessageW(g_hw,WM_ATK,(WPARAM)lock_oid,0);}
      else{ka++;if(ka>=6){ka=0;L("[獵]走位卡住補點 objid=0x%X 距=%ld",lock_oid,dd);PostMessageW(g_hw,WM_ATK,(WPARAM)lock_oid,0);}}
     }
    }
    else if(!lock_oid){
     /* 沒目標: 挑最近活著通暢怪, 認定牠 */
     long bd;uint32_t best=nearest_clear2(pl,px,py,1,&bd);
     if(best){lock_oid=RU(best+OFF_ID);ka=0;lostc=0;
      L("[獵]新目標0x%08X objid=0x%X 距=%ld →打到死",best,lock_oid,bd);PostMessageW(g_hw,WM_ATK,(WPARAM)lock_oid,0);}
    }
