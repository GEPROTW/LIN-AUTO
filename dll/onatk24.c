...
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk24.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk24_cmd.txt"
...
static int g_view=30;  /* 畫面範圍(格子): 目標涵蓋範圍. 指令 view<n> 即時調 */
...
static void cmd(void){FILE*f=fopen(CMDPATH,"r");if(!f)return;char b[64]={0};fgets(b,64,f);fclose(f);remove(CMDPATH);
 ...
 else if(strncmp(b,"view",4)==0){int v=atoi(b+4);if(v>=5&&v<=150){g_view=v;L("[CMD]畫面範圍 g_view=%d 格",g_view);}else L("[CMD]view 數值需 5~150");}
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk24 啟動 pid=%lu ====",GetCurrentProcessId());
