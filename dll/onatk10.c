...
#define OFF_ID 0x0C
#define OFF_AI 0x10
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk10.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk10_cmd.txt"
...
/* 最近的「距離<=MAXDIST 且視線通暢」怪. want_clear=0 時不做視線過濾 */
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk10 啟動 pid=%lu ====",GetCurrentProcessId());
 ...
 for(;;){Sleep(150);cmd();
  ...
  /* 自動打怪(移動感知): 點一次讓遊戲自己走去打, 移動途中絕不重複點(避免指令衝突崩潰)。
     只有角色「停下來」且怪還活著才補點; 怪死/太遠/被牆擋就換最近通暢怪。 */
  if(hunt&&g_hw){htick++;if(htick>=3){htick=0;   /* 每~450ms 檢查 */
   ...
   if(e&&(dd>MAXDIST||!los_clear(px,py,RI(e+OFF_X),RI(e+OFF_Y))))e=0;  /* 太遠/被擋→放棄 */
   if(e){
    if(moving){ka=0;/* 移動中: 讓遊戲自己走, 不重複點 */}
    else if(dd<=BOWRANGE){ka=0;/* 進射程且停下=正在射: 絕不補點(補點會把弓手往前推進近戰) */}
    else{ka++;if(ka>=6){ka=0;/* 停下但目標還很遠=走位卡住: 才補點恢復接近(每~2.7秒) */
      L("[獵]走位卡住補點 objid=0x%X 距=%ld",lock_oid,dd);PostMessageW(g_hw,WM_ATK,(WPARAM)lock_oid,0);}}
   }
   else{ uint32_t n=nearest_clear(pl,px,py,1);
     if(n){lock_oid=RU(n+OFF_ID);ka=0;
      L("[獵]新目標0x%08X objid=0x%X @(%d,%d) 距=%ld →走去打",n,lock_oid,RI(n+OFF_X),RI(n+OFF_Y),labs(RI(n+OFF_X)-px)+labs(RI(n+OFF_Y)-py));
      PostMessageW(g_hw,WM_ATK,(WPARAM)lock_oid,0);}
     else{lock_oid=0;/* 附近無怪, 靜候(不洗log) */} }
  }}
 }return 0;}
