...
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk11.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk11_cmd.txt"
...
#define BOWRANGE 12  /* 弓射程內: 停下射擊, 絕不再補點(避免把弓手推進近戰崩潰) */
...
static void diag(void){uint32_t pl=RU(PLAYER);if(!pl){L("[診斷]未進場");return;}if(!farr(pl)){L("[診斷]無怪陣列");return;}
 ...
 for(int c=0;c<8;c++){uint32_t best=0;long bd=1<<30;
  ...
  L("[診斷]怪%d 0x%08X objid=0x%X @(%d,%d) 距=%ld 牆=%d 視線=%d 狀態[+14]=%d 活=%d",
    c,best,RU(best+OFF_ID),mx,my,bd,wall(mx,my),los_clear(px,py,mx,my),RB(best+OFF_STATE),alive(best));}
}
static int mode=0,hunt=0;static uint32_t lock_oid=0;static int htick=0,ka=0;static int lastpx=0,lastpy=0;
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk11 啟動 pid=%lu ====",GetCurrentProcessId());
 ...
 for(;;){Sleep(150);cmd();
  ...
  /* 自動打怪(就近優先+移動感知+死亡偵測):
     - 每輪找最近活著通暢怪 best; 鎖定目標死/太遠/被擋就換; 出現明顯更近的(近MARGIN格)也換
     - 移動中不點; 進射程停下=正在射不補點(避免推進近戰); 停下但還遠=走位卡住才補點
     - 換目標/補點都只在「停下時」做(避免移動中送指令衝突崩潰) */
  if(hunt&&g_hw){htick++;if(htick>=3){htick=0;   /* 每~450ms 檢查 */
   ...
   long dd=e?labs(RI(e+OFF_X)-px)+labs(RI(e+OFF_Y)-py):999;
   if(e&&(dd>MAXDIST||!los_clear(px,py,RI(e+OFF_X),RI(e+OFF_Y)))){e=0;dd=999;}
   /* 找最近活著通暢怪 */
   long bd;uint32_t best=nearest_clear2(pl,px,py,1,&bd);
   if(moving){ka=0;/* 移動中: 全部維持現狀, 讓遊戲自己走, 絕不送新指令 */}
   else{
    /* 停下: 決定要打誰. 無鎖定, 或最近怪比鎖定的近 MARGIN 格以上 → 切換到最近怪 */
    if(best&&(!e||bd+3<=dd)){
     if(RU(best+OFF_ID)!=lock_oid){lock_oid=RU(best+OFF_ID);ka=0;
      L("[獵]打最近怪0x%08X objid=0x%X 距=%ld%s",best,lock_oid,bd,e?"(換更近)":"");
      PostMessageW(g_hw,WM_ATK,(WPARAM)lock_oid,0);e=best;dd=bd;}
     else{e=best;dd=bd;}
    }
    if(e){ /* 有鎖定目標且停下 */
     if(dd<=BOWRANGE){ka=0;/* 進射程正在射: 不補點 */}
     else{ka++;if(ka>=6){ka=0;/* 走位卡住: 補點恢復接近 */
       L("[獵]走位卡住補點 objid=0x%X 距=%ld",lock_oid,dd);PostMessageW(g_hw,WM_ATK,(WPARAM)lock_oid,0);}}
    }
    else lock_oid=0; /* 附近無怪, 靜候 */
   }
  }}
 }return 0;}
