...
#define OFF_ID 0x0C
#define OFF_AI 0x10
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk9.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk9_cmd.txt"
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk9 啟動 pid=%lu ====",GetCurrentProcessId());
 ...
 for(;;){Sleep(150);cmd();
  ...
  if(hunt&&g_hw){htick++;if(htick>=3){htick=0;   /* 每~450ms 檢查 */
   ...
   if(e){
    ...
    else{ka++;if(ka>=5){ka=0;/* 停下且怪還在: 補點一次(每~2.3秒) */
      L("[獵]補點 objid=0x%X 距=%ld",lock_oid,dd);PostMessageW(g_hw,WM_ATK,(WPARAM)lock_oid,0);}}
   }
