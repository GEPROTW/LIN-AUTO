...
#define OFF_ID 0x0C
#define OFF_AI 0x10
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk5.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk5_cmd.txt"
...
static void do_native(uint32_t e){uint32_t pl=RU(PLAYER);if(!pl||!heap(e))return;
 if(rdok(TARGET,4))*(uint32_t*)TARGET=e;if(rdok(HOVERSEL,4))*(uint32_t*)HOVERSEL=e;
 L("[主]FUN_0040e790(怪0x%08X objid=0x%X)",e,RU(e+OFF_ID));
 ((actfn_t)FN_ACT)((int)e);L("[主]返回");}
static void do_send22(uint32_t e){if(!heap(e))return;uint32_t oid=RU(e+OFF_ID);
 if(rdok(TARGET,4))*(uint32_t*)TARGET=e;
 L("[主]直送0x22(怪0x%08X objid=0x%X)",e,oid);
 ((sendfn_t)FN_SEND)(FMT_022,0x22,(int)oid);L("[主]直送返回");}
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk5 啟動 pid=%lu ====",GetCurrentProcessId());
  ...
  if(mode){int md=mode;mode=0;
   if(md==9){diag();}
    ...
    if(mo){L("[單發]最近通暢怪0x%08X objid=0x%X @(%d,%d) 距=%ld",mo,RU(mo+OFF_ID),RI(mo+OFF_X),RI(mo+OFF_Y),labs(RI(mo+OFF_X)-px)+labs(RI(mo+OFF_Y)-py));
     PostMessageW(g_hw,md==1?WM_ATK:WM_S22,(WPARAM)mo,0);}else L("[單發]無視線通暢的怪");}}}
  /* 自動打怪(保守版): 每隻怪只點一次讓遊戲自己打, 怪死了(消失)才換下一隻,
     鎖定中每~4秒補點一次(keepalive). 大幅降低呼叫頻率避免踢線/崩潰。 */
  if(hunt&&g_hw){htick++;if(htick>=4){htick=0;   /* 每~600ms 檢查一次 */
   ...
   int blocked=(e&&!los_clear(px,py,RI(e+OFF_X),RI(e+OFF_Y)));
   if(e&&!blocked){
    /* 目標還活著且視線通暢: 讓遊戲繼續自己打, 只每~4秒補點一次 */
    ka++;if(ka>=7){ka=0;L("[獵keepalive]objid=0x%X 距=%ld",lock_oid,labs(RI(e+OFF_X)-px)+labs(RI(e+OFF_Y)-py));
     PostMessageW(g_hw,WM_ATK,(WPARAM)e,0);}
   } else {
    /* 目標死亡/消失/被牆擋: 換最近通暢怪, 點一次 */
    uint32_t n=nearest_clear(pl,px,py,1);
    if(n){lock_oid=RU(n+OFF_ID);ka=0;
     L("[獵]新目標0x%08X objid=0x%X @(%d,%d) 距=%ld → 點一次",n,lock_oid,RI(n+OFF_X),RI(n+OFF_Y),labs(RI(n+OFF_X)-px)+labs(RI(n+OFF_Y)-py));
     PostMessageW(g_hw,WM_ATK,(WPARAM)n,0);}
