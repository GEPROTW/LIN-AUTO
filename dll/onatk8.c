...
#define OFF_ID 0x0C
#define OFF_AI 0x10
#define LOGPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk8.log"
#define CMDPATH "C:\\Users\\pc\\orca\\LIN-AUTO\\dumps\\onatk8_cmd.txt"
...
#define MAXDIST 10   /* 只打曼哈頓距離 <= 此值的近怪 */
...
static DWORD WINAPI wk(LPVOID _){(void)_;g=fopen(LOGPATH,"a");L("==== onatk8 啟動 pid=%lu ====",GetCurrentProcessId());
 ...
 for(;;){Sleep(150);cmd();
  ...
  /* 自動打怪: 只打距離<=10且視線通暢的近怪, FUN_0040e790, 保守點擊(每隻點一次讓遊戲自己打,
     鎖定中每~4秒補點, 距離>10或視線被擋或死亡就換). 近怪在射程內→站著射不用追→避免撞牆/近戰崩潰。 */
  if(hunt&&g_hw){htick++;if(htick>=4){htick=0;   /* 每~600ms 檢查 */
   ...
   if(e){ ka++;if(ka>=7){ka=0;L("[獵keepalive]objid=0x%X 距=%ld",lock_oid,dd);
     ...
     if(n){lock_oid=RU(n+OFF_ID);ka=0;
      ...
      PostMessageW(g_hw,WM_ATK,(WPARAM)lock_oid,0);}
     else{lock_oid=0;L("[獵]附近(<=10)無視線通暢的怪, 等待");} }
  }}
 }return 0;}
